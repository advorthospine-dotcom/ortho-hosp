...
<div class="min-h-screen bg-slate-50/60 pb-20">
    ...
    <div class="relative bg-gradient-to-br from-slate-950 via-[#0a2f3c] to-slate-950 text-white overflow-hidden py-16 sm:py-20 border-b border-slate-800">
        ...
        <div class="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12 relative z-10 text-center space-y-5">
            <span class="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full text-xs font-bold bg-[#114b5f]/40 text-teal-300 border border-teal-400/30 tracking-wider uppercase shadow-inner">
                <i class="ri-shield-cross-fill text-teal-300"></i> Clinical Specialties & Surgical Excellence
            </span>
    ...
    <div class="max-w-[1440px] mx-auto px-4 sm:px-8 lg:px-12 py-10 sm:py-14 space-y-10">
        ...
        <div class="bg-white border border-slate-200/80 p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-sm space-y-4">
            ...
            <!-- Category Pills Navigation Bar (Soft Light Tint Active State) -->
            <div class="flex overflow-x-auto sm:flex-wrap items-center gap-2.5 pb-2 sm:pb-0 scrollbar-none [::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                ...
                <button wire:click="selectCategory('all')" 
                        class="px-4 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer inline-flex items-center gap-2 shrink-0 @if($activeCategory === 'all') bg-teal-50 text-[#114b5f] border-2 border-[#114b5f] font-extrabold shadow-2xs @else bg-slate-100/80 hover:bg-slate-200/70 text-slate-700 hover:text-[#114b5f] border border-slate-200/60 @endif">
                    <i class="ri-apps-2-line text-sm"></i>
                    <span>All Specialties</span>
                    <span class="px-2 py-0.5 text-[10px] rounded-full font-semibold @if($activeCategory === 'all') bg-[#114b5f] text-white @else bg-slate-200 text-slate-600 @endif">
                        {{ $this->categoryCounts['all'] ?? 0 }}
                    </span>
                </button>

                <button wire:click="selectCategory('trauma')" 
                        class="px-4 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer inline-flex items-center gap-2 shrink-0 @if($activeCategory === 'trauma') bg-rose-50 text-rose-700 border-2 border-rose-600 font-extrabold shadow-2xs @else bg-slate-100/80 hover:bg-slate-200/70 text-slate-700 hover:text-[#114b5f] border border-slate-200/60 @endif">
                    <i class="ri-alarm-warning-line text-sm text-rose-500"></i>
                    <span>Trauma & Emergency</span>
                    <span class="px-2 py-0.5 text-[10px] rounded-full font-semibold @if($activeCategory === 'trauma') bg-rose-600 text-white @else bg-slate-200 text-slate-600 @endif">
                        {{ $this->categoryCounts['trauma'] ?? 0 }}
                    </span>
                </button>

                <button wire:click="selectCategory('spine')" 
                        class="px-4 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer inline-flex items-center gap-2 shrink-0 @if($activeCategory === 'spine') bg-teal-50 text-[#114b5f] border-2 border-[#114b5f] font-extrabold shadow-2xs @else bg-slate-100/80 hover:bg-slate-200/70 text-slate-700 hover:text-[#114b5f] border border-slate-200/60 @endif">
                    <i class="ri-pulse-line text-sm text-teal-600"></i>
                    <span>Spine & Back Care</span>
                    <span class="px-2 py-0.5 text-[10px] rounded-full font-semibold @if($activeCategory === 'spine') bg-[#114b5f] text-white @else bg-slate-200 text-slate-600 @endif">
                        {{ $this->categoryCounts['spine'] ?? 0 }}
                    </span>
                </button>

                <button wire:click="selectCategory('joints')" 
                        class="px-4 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer inline-flex items-center gap-2 shrink-0 @if($activeCategory === 'joints') bg-teal-50 text-[#114b5f] border-2 border-[#114b5f] font-extrabold shadow-2xs @else bg-slate-100/80 hover:bg-slate-200/70 text-slate-700 hover:text-[#114b5f] border border-slate-200/60 @endif">
                    <i class="ri-robot-2-line text-sm text-teal-600"></i>
                    <span>Joint Replacements</span>
                    <span class="px-2 py-0.5 text-[10px] rounded-full font-semibold @if($activeCategory === 'joints') bg-[#114b5f] text-white @else bg-slate-200 text-slate-600 @endif">
                        {{ $this->categoryCounts['joints'] ?? 0 }}
                    </span>
                </button>

                <button wire:click="selectCategory('sports')" 
                        class="px-4 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer inline-flex items-center gap-2 shrink-0 @if($activeCategory === 'sports') bg-emerald-50 text-[#3b774b] border-2 border-[#3b774b] font-extrabold shadow-2xs @else bg-slate-100/80 hover:bg-slate-200/70 text-slate-700 hover:text-[#3b774b] border border-slate-200/60 @endif">
                    <i class="ri-football-line text-sm text-emerald-600"></i>
                    <span>Sports Medicine</span>
                    <span class="px-2 py-0.5 text-[10px] rounded-full font-semibold @if($activeCategory === 'sports') bg-[#3b774b] text-white @else bg-slate-200 text-slate-600 @endif">
                        {{ $this->categoryCounts['sports'] ?? 0 }}
                    </span>
                </button>

                <button wire:click="selectCategory('specialized')" 
                        class="px-4 py-2.5 text-xs font-bold rounded-xl transition-all cursor-pointer inline-flex items-center gap-2 shrink-0 @if($activeCategory === 'specialized') bg-emerald-50 text-[#3b774b] border-2 border-[#3b774b] font-extrabold shadow-2xs @else bg-slate-100/80 hover:bg-slate-200/70 text-slate-700 hover:text-[#3b774b] border border-slate-200/60 @endif">
                    <i class="ri-run-line text-sm text-emerald-600"></i>
                    <span>Specialized & Rehab</span>
                    <span class="px-2 py-0.5 text-[10px] rounded-full font-semibold @if($activeCategory === 'specialized') bg-[#3b774b] text-white @else bg-slate-200 text-slate-600 @endif">
                ...
                <div class="bg-white border border-slate-200/80 rounded-2xl overflow-hidden flex flex-col justify-between shadow-[0_4px_20px_rgba(0,0,0,0.03)] hover:shadow-[0_16px_32px_rgba(17,75,95,0.08)] hover:border-[#114b5f]/40 transition-all duration-300 group relative">
                    ...
                    <div class="p-6 space-y-5 flex-1 flex flex-col justify-between">
                        <div class="space-y-4">
                            <!-- Title & Description -->
                            <div class="space-y-2">
                                <h3 class="text-base sm:text-lg font-heading font-extrabold text-slate-900 group-hover:text-[#114b5f] transition-colors duration-200 leading-snug">
                                ...
                                <ul class="space-y-2 pt-3 border-t border-slate-100">
                                    @foreach($features as $feat)
                                        <li class="flex items-start gap-2 text-slate-600 text-xs">
                                            <i class="ri-checkbox-circle-fill text-[#114b5f] text-xs shrink-0 mt-0.5"></i>
        ...
        <div class="bg-white border border-slate-200/80 rounded-2xl sm:rounded-3xl p-6 sm:p-8 shadow-sm">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 text-center divide-y md:divide-y-0 md:divide-x divide-slate-100">
                <div class="space-y-1 p-2">
                    <div class="text-2xl text-[#114b5f] font-extrabold flex justify-center">
                ...
                <div class="space-y-1 p-2 pt-4 md:pt-2">
                    <div class="text-2xl text-[#3b774b] font-extrabold flex justify-center">
