<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200/80 pb-5">
        <div>
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-2xl font-heading font-bold text-slate-900 tracking-tight">Edit Clinical Service</h1>
            </div>
            <p class="text-slate-500 text-sm mt-1">Update medical procedure details, treatment features checklist, SEO tags, and featured image.</p>
        </div>
        ...
        <div class="flex items-center gap-2">
            ...
            <button type="button" 
                    @click="$refs.serviceUpdateForm.requestSubmit()"
    ...
    <!-- Main Grid Form -->
    <form x-ref="serviceUpdateForm" wire:submit.prevent="save" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm space-y-5">
                ...
                <div class="space-y-1.5"
                     x-data="{
                         ...
                         initTinyMCE() {
                             let init = () => {
                                 ...
                                 if (tinymce.get($refs.updateEditor.id)) {
                                     tinymce.get($refs.updateEditor.id).destroy();
                                 }
                                 tinymce.init({
                                     target: $refs.updateEditor,
                                     height: 420,
                         ...
                         destroy() {
                             if (typeof tinymce !== 'undefined' && tinymce.get($refs.updateEditor.id)) {
                                 tinymce.get($refs.updateEditor.id).destroy();
                             }
                         }
                     }" 
                    ...
                    <textarea x-ref="updateEditor" id="update-desc" class="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none"></textarea>
                ...
                <div class="space-y-3">
                    <div class="relative border-2 border-dashed border-slate-200 rounded-2xl p-4 flex flex-col items-center justify-center hover:border-[#114b5f] transition-colors text-center group cursor-pointer">
                        ...
                        <div class="space-y-1.5 py-4">
                            ...
                            <p class="text-xs font-semibold text-slate-600">Click to replace image</p>
                    ...
                    @elseif ($existingImage)
                        <div class="space-y-2 border border-slate-150 p-2 rounded-xl bg-slate-50 relative group">
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Current Image:</span>
                            <div class="aspect-video w-full rounded-lg overflow-hidden border border-slate-200 relative">
                                <img src="{{ Str::startsWith($existingImage, 'http') ? $existingImage : asset('storage/'.$existingImage) }}" class="w-full h-full object-cover" alt="Current Image" />
                            </div>
                            <button type="button" 
                                    wire:click="removeImage" 
                                    class="w-full inline-flex items-center justify-center gap-1 px-3 py-1.5 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-600 text-xs font-semibold border border-rose-200/60 transition-all cursor-pointer">
